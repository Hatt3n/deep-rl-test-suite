from deps.SLM_Lab.dansah_custom.memory.base import Memory
...
class OnPolicyReplay(Memory):
    ...
    An experience consists of
        ...
        - rft: 0 / 1 representing if the reason for termination was a timeout (i.e. 1 = timeout)
    ...
    def __init__(self, memory_spec, body):
        ...
        self.data_keys = ['states', 'actions', 'rewards', 'next_states', 'dones', 'rfts'] # NOTE: Added "rfts" (reason for termination(s)), @dansah
        self.reset()
    ...
    @lab_api
    def update(self, state, action, reward, next_state, done, rft): # NOTE: Added "rft" (reason for termination), @dansah
        '''Interface method to update memory'''
        self.add_experience(state, action, reward, next_state, done, rft)

    def add_experience(self, state, action, reward, next_state, done, rft): # NOTE: Added "rft" (reason for termination), @dansah
        '''Interface helper method for update() to add experience to memory'''
        self.most_recent = (state, action, reward, next_state, done, rft)
