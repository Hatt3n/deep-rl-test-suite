...
class Agent:
    ...
    def __init__(self, spec, body, global_nets=None):
        ...
        AlgorithmClass = getattr(dansah_custom, ps.get(self.agent_spec, 'algorithm.name'))
...
class Body:
    ...
    def calc_df_row(self, env):
        ...
        # TODO: Some or all of the commented values now forced to 0 should be printable.
        row = pd.Series({
            ...
            'loss': self.loss,
            'lr': 0, #self.get_mean_lr(),
            'explore_var': 0, #self.explore_var,
            'entropy_coef': 0, # self.entropy_coef if hasattr(self, 'entropy_coef') else np.nan,
            'entropy': 0, # self.mean_entropy,
            'grad_norm': 0 # self.mean_grad_norm,
        }, dtype=np.float32)
