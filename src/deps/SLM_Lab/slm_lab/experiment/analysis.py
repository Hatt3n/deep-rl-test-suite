...
from deps.SLM_Lab.slm_lab.spec import random_baseline
import numpy as np
import pandas as pd
import pydash as ps
import shutil
...
def calc_session_metrics(session_df, env_name, info_prepath=None, df_mode=None, skip_random_baseline=False):
    '''
...
def analyze_session(session_spec, session_df, df_mode, skip_random_baseline=False, plot=True):
    ...
    # calculate metrics
    session_metrics = calc_session_metrics(session_df, ps.get(session_spec, 'env.0.name'), info_prepath, df_mode, skip_random_baseline=skip_random_baseline)
    if plot:
        # plot graph
